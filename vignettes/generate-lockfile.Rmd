...
repo_urls <- tibble(repo_urls = parsedFile$`Repo URLs`[[1]]) %>%
  separate_wider_delim(cols = repo_urls, delim = ":  ", names = c("repo_name", "repo_url"))
...
lockfile_repos <- repo_urls$repo_name
names(lockfile_repos) <- repo_urls$repo_url
