...
#'   defaults to TRUE
#' @param quit_on_error Should the session quit with status of 1 with error,
#'   defaults to TRUE
...
#'   stream: a complete stream of any output, errors, warnings, and messages
...
axecute <- function(file,
   ...
   match.arg(to_report, c("messages", "output", "result", "stream"), several.ok = TRUE)

   # check if both output and stream are in to_report
   if (all(c("output", "stream") %in% to_report)) {
      stop("to_report param can not support both output and result options at the same time")
   }
   ...
   # run the code
   run_safely_loudly(file, to_report)
